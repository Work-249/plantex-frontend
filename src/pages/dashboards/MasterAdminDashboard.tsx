...
const MasterAdminDashboard: React.FC<MasterAdminDashboardProps> = ({ activeTab, setActiveTab }) => {
  ...
  const handleCreateCollege = async (collegeData: {
    ...
    try {
      setFormLoading(true);
      const resp: any = await apiService.createCollege(collegeData);
      // Notify success
      toast.success(`College "${collegeData.name}" created successfully`);
      console.log(`[college] Created: ${collegeData.name} (${collegeData.code})`);
      setShowCollegeForm(false);
      await loadData();
    } catch (error) {
      // Provide helpful console output and show toast
      const msg = error instanceof Error ? error.message : String(error);
      console.error('Create college error:', msg);
      if (msg.includes('same code or email')) {
        console.error('This error means a college with that code or email already exists. Choose a unique code/email.');
      }
      toast.error(`Failed to create college: ${msg}`);
      throw error;
    } finally {
