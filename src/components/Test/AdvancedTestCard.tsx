...
import {Activity , Clock, Calendar, FileText, Eye, Send, CreditCard as Edit2, Trash2, BarChart3, Award,  Target, CheckCircle, ChevronDown, ChevronUp, Info, PlayCircle } from 'lucide-react';
import { useTestResume } from '../../contexts/TestResumeContext';
...
const AdvancedTestCard: React.FC<AdvancedTestCardProps> = ({
  ...
  const { hasUnsavedProgress } = useTestResume();
      ...
      <div className="p-6 space-y-4">
        ...
        {showActions && (
          <div className="space-y-2 pt-2">
            {hasUnsavedProgress(test._id) && (
              <div className="w-full bg-amber-50 border-l-4 border-amber-500 p-3 rounded-lg flex items-start gap-2">
                <div className="flex-1">
                  <p className="text-xs font-semibold text-amber-800">Unsaved Progress Detected</p>
                  <p className="text-xs text-amber-700">You have a previous attempt in progress. Resume to continue.</p>
                </div>
              </div>
            )}
            <div className="grid grid-cols-2 gap-2">
              {hasUnsavedProgress(test._id) && (
                <button
                  onClick={() => onView(test._id)}
                  className="bg-gradient-to-r from-amber-500 to-amber-600 text-white py-3 px-4 rounded-xl hover:from-amber-600 hover:to-amber-700 flex items-center justify-center gap-2 text-sm font-semibold shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-200 col-span-2"
                >
                  <PlayCircle size={16} />
                  Resume Test
                </button>
              ...
              <button
                onClick={() => onView(test._id)}
                className={`${hasUnsavedProgress(test._id) ? 'hidden' : ''} bg-gradient-to-r from-blue-500 to-blue-600 text-white py-3 px-4 rounded-xl hover:from-blue-600 hover:to-blue-700 flex items-center justify-center gap-2 text-sm font-semibold shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-200`}
              >
                <Eye size={16} />
                Preview
              </button>
              {onEdit && !hasUnsavedProgress(test._id) && (
                <button
              ...
              {onDelete && !hasUnsavedProgress(test._id) && (
                <button
            ...
            {onReport && !hasUnsavedProgress(test._id) && (
              <button
