...
const TCSTestInterface: React.FC<TCSTestInterfaceProps> = ({
  ...
  const { saveTestProgress } = useTestResume();
    ...
    if (showInstructions) {
      ...
      if (savedProgress) {
        try {
          ...
          setVisited(new Set(progress.visited || []));
  ...
  }, [violations, showInstructions, submitting, mcqCompleted, answers, test._id, startTime, onSubmit, onExit]);
  ...
  // Save test progress when user exits or page unloads
  useEffect(() => {
    const handleBeforeUnload = () => {
      if (!showInstructions && !submitting) {
        // Save progress to context
        saveTestProgress(test._id, {
          answers,
  ...
  }, [answers, markedForReview, visited, currentSectionIndex, currentQuestionIndex, violations, sectionTimeLeft, mcqCompleted, selectedCodingQuestionId, showInstructions, submitting, test._id, startTime, saveTestProgress]);
  ...
  const handleStartTest = () => {
    ...
    // Check if this is a coding-only test (no MCQ questions in any section)
    let hasMcqQuestions = false;
    if (test.hasSections && test.sections && test.sections.length > 0) {
      hasMcqQuestions = test.sections.some(section => section.questions && section.questions.length > 0);
    } else {
      hasMcqQuestions = test.questions && test.questions.length > 0;
    }

    // If no MCQ questions but has coding section, go straight to coding
    if (!hasMcqQuestions && test.hasCodingSection && test.codingQuestions && test.codingQuestions.length > 0) {
      console.log('🎯 Coding-only test detected (TCSTestInterface) - showing coding section immediately');
      setMcqCompleted(true);
      if (test.codingQuestions.length > 0) {
        setSelectedCodingQuestionId(test.codingQuestions[0]._id || test.codingQuestions[0].questionId);
      }
  ...
  const handleNextSection = async () => {
    ...
    if (test.hasSections && test.sections && currentSectionIndex < test.sections.length - 1) {
      ...
      if (test.hasCodingSection && test.codingQuestions && test.codingQuestions.length > 0) {
        console.log('✅ MCQ answers will be submitted before transitioning to coding section');
        ...
        console.log('❌ No coding section. Showing final submit confirmation.');
      ...
      <div className="min-h-screen bg-gray-100 flex flex-col">
        <div className="bg-white shadow-md border-b-2 border-green-600 px-6 py-3">
          <div className="flex justify-between items-center">
            ...
            <button
              onClick={() => {
                console.log('✅ Submitting test from coding section');
                setShowSubmitConfirm(true);
              }}
