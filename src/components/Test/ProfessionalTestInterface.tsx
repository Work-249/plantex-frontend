...
import { Clock, CheckCircle, AlertTriangle, Bookmark, ChevronRight, ChevronLeft, List, X, Monitor, Shield, Eye, AlertCircle, Code } from 'lucide-react';
...
const ProfessionalTestInterface: React.FC<ProfessionalTestInterfaceProps> = ({
  ...
  const getTestTotals = () => {
    // Calculate MCQ totals from sections if present, otherwise use test-level totals
    // This should NOT include coding questions - they are separate
    if (test.hasSections && test.sections && test.sections.length > 0) {
      const totalQuestions = test.sections.reduce((sum, section) => sum + section.numberOfQuestions, 0);
      const totalMarks = test.sections.reduce((sum, section) => sum + (section.numberOfQuestions * section.marksPerQuestion), 0);
      const totalDuration = test.sections.reduce((sum, section) => sum + section.sectionDuration, 0);
      
      return {
        mcqQuestions: totalQuestions,
        mcqMarks: totalMarks,
        duration: totalDuration,
        sections: test.sections.length,
        codingQuestions: test.codingQuestions ? test.codingQuestions.length : 0,
      };
    }
    
    return {
      mcqQuestions: test.numberOfQuestions,
      mcqMarks: test.totalMarks,
  ...
  const handleSubmit = async (isAutoSubmit = false) => {
    console.debug('ProfessionalTestInterface.handleSubmit called', { isAutoSubmit });
    if (submitting) {
      console.debug('Professional handleSubmit: already submitting, ignoring');
      return;
    }
    ...
    try {
      console.debug('Professional handleSubmit: starting submit, setting submitting=true');
      setSubmitting(true);
      ...
      if (test.hasCodingSection && test.codingQuestions && test.codingQuestions.length > 0 && startCodingFlow) {
        console.debug('Professional handleSubmit: submitting MCQ before coding', { submissionAnswersLength: submissionAnswers.length, timeSpent, tabSwitches });
      ...
      console.debug('Professional handleSubmit: calling onSubmit final', { submissionAnswersLength: submissionAnswers.length, timeSpent, tabSwitches });
      await onSubmit(submissionAnswers, timeSpent, tabSwitches);
      console.debug('Professional handleSubmit: onSubmit resolved');
      setSubmitting(false);
  ...
  // Force final submission for the entire test (used when user chooses to skip coding)
  const handleFinalSubmit = async () => {
    if (submitting) return;

    console.debug('Professional.handleFinalSubmit called');
    try {
      console.debug('Professional handleFinalSubmit: starting submit');
      setSubmitting(true);
      ...
      console.debug('Professional handleFinalSubmit: calling onSubmit', { submissionAnswersLength: submissionAnswers.length, timeSpent, tabSwitches });
      await onSubmit(submissionAnswers, timeSpent, tabSwitches);
      console.debug('Professional handleFinalSubmit: onSubmit resolved');
      setSubmitting(false);
      onExit();
    } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100 flex items-center justify-center p-4">
        <div className="max-w-4xl w-full bg-white rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="p-8">
            <div className={`mb-8 grid gap-4 ${test.hasCodingSection && test.codingQuestions && test.codingQuestions.length > 0 ? 'grid-cols-5' : 'grid-cols-4'}`}>
              <div className="bg-blue-50 p-4 rounded-lg text-center border border-blue-200">
                <div className="text-2xl font-bold text-blue-600">{getTestTotals().mcqQuestions}</div>
                <div className="text-sm text-gray-600">MCQ Questions</div>
              </div>
              {test.hasSections && test.sections && test.sections.length > 0 && (
                <div className="bg-indigo-50 p-4 rounded-lg text-center border border-indigo-200">
                  <div className="text-2xl font-bold text-indigo-600">{getTestTotals().sections}</div>
                  <div className="text-sm text-gray-600">Sections</div>
                </div>
              )}
              <div className="bg-green-50 p-4 rounded-lg text-center border border-green-200">
                <div className="text-2xl font-bold text-green-600">{getTestTotals().duration}</div>
                <div className="text-sm text-gray-600">Minutes</div>
              </div>
              {test.hasCodingSection && test.codingQuestions && test.codingQuestions.length > 0 && (
                <div className="bg-purple-50 p-4 rounded-lg text-center border border-purple-200">
                  <div className="text-2xl font-bold text-purple-600">{getTestTotals().codingQuestions}</div>
                  <div className="text-sm text-gray-600">Coding Questions</div>
                </div>
              )}
              <div className={`${test.hasCodingSection && test.codingQuestions && test.codingQuestions.length > 0 ? 'bg-orange-50 border-orange-200' : 'bg-purple-50 border-purple-200'} p-4 rounded-lg text-center border`}>
                <div className={`text-2xl font-bold ${test.hasCodingSection && test.codingQuestions && test.codingQuestions.length > 0 ? 'text-orange-600' : 'text-purple-600'}`}>{getTestTotals().mcqMarks}</div>
                <div className="text-sm text-gray-600">MCQ Marks</div>
              </div>
            </div>
              ...
              <div className="space-y-4">
                ...
                {test.hasCodingSection && test.codingQuestions && test.codingQuestions.length > 0 && (
                  <div className="bg-purple-50 border-l-4 border-purple-500 p-4 rounded">
                    ...
                    <ul className="list-disc list-inside space-y-2 text-gray-700 text-sm">
                      <li>This test includes <strong>{test.codingQuestions.length} coding question(s)</strong></li>
                      <li>Write code in your preferred programming language (Python, JavaScript, Java, C++)</li>
                      <li>Your code will be executed against multiple test cases</li>
                      <li>Click <strong>"Run Code"</strong> to test your solution before submitting</li>
                      <li>Click <strong>"Submit"</strong> to finalize your coding solution</li>
                      <li>After submitting the coding section, you will move to the next question or complete the test</li>
                    </ul>
                  </div>
                )}
        ...
        <div className="px-6 py-3">
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center gap-6">
              ...
              <button
                onClick={() => {
                  // If there's a coding section, automatically enable the coding flow
                  if (test.hasCodingSection && test.codingQuestions && test.codingQuestions.length > 0) {
                    setStartCodingFlow(true);
                  }
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-8 max-w-md w-full mx-4 shadow-2xl">
              ...
              {['Assessment', 'Mock Test', 'Specific Company Test'].includes(test.testType || '') && (
                <div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-r-lg">
                  <p className="text-sm text-blue-800 font-medium">
                    Time Remaining: <span className="font-bold">{formatTime(currentTime)}</span>
                  </p>
                </div>
              )}
            ...
            <div className="flex justify-end gap-3">
              <button
                onClick={() => { console.debug('Professional modal: Cancel clicked'); setShowConfirmSubmit(false); if (startCodingFlow) setStartCodingFlow(false); }}
                disabled={submitting}
              ...
              {startCodingFlow && (
                <button
                  ...
                  disabled={submitting}
                  className="px-6 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 disabled:bg-gray-400 font-medium flex items-center gap-2"
                >
                  {submitting ? (
                    <>
              ...
              <button
                onClick={() => { console.debug('Professional modal: Confirm clicked'); handleSubmit(); }}
                disabled={submitting}
