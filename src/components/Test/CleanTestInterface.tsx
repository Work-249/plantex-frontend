...
import styles from './CleanTestInterface.module.css';
...
const CleanTestInterface: React.FC<CleanTestInterfaceProps> = ({
  ...
  // No change needed here except to ensure a return statement is present at the end of the function.
  ...
  // Handle tab switch/visibility change to increment tabSwitches
  const handleVisibilityChange = () => {
    if (document.visibilityState === 'hidden') {
      setTabSwitches((prev) => prev + 1);
      alert('⚠️ Warning: You switched tabs or minimized the window! Please stay on the test page.');
    }
  };
  ...
  useEffect(() => {
    ...
    const handleFullscreenChange = () => {
      if (!document.fullscreenElement && !showInstructions) {
        ...
        setFullscreenViolations((prev) => {
          ...
          if (newCount >= 3) {
            alert('You have exited fullscreen mode 3 times. The test will now be submitted.');
            handleAutoSubmit();
          }
          return newCount;
        });
    ...
    // You need to define handleVisibilityChange or ensure it's defined elsewhere
  ...
  const handleStartTest = async () => {
    ...
    const hasMCQQuestions = currentQs && currentQs.length > 0;
    const hasCodingQuestions = test.hasCodingSection && test.codingQuestions && test.codingQuestions.length > 0;
    
    // If it's a coding-only test (no MCQ questions), skip directly to coding section
    if (!hasMCQQuestions && hasCodingQuestions) {
      setMcqCompleted(true);
      const first = test.codingQuestions && test.codingQuestions[0];
      const firstCodingId = first ? (first._id || first.questionId || first.id) : undefined;
      if (firstCodingId) {
        setSelectedCodingQuestionId(firstCodingId);
      }
    } else if (hasMCQQuestions) {
      // Standard MCQ test or MCQ + Coding test - show MCQ first
  ...
  const handleSubmit = useCallback(async (isAutoSubmit: boolean = false) => {
    console.debug('CleanTestInterface.handleSubmit called', { isAutoSubmit });
    if (submitting) {
      console.debug('handleSubmit: already submitting, ignoring');
      return;
    }
    const counts = getCounts();

    console.debug('handleSubmit counts', counts);
    ...
    try {
      console.debug('handleSubmit: starting submit, setting submitting=true');
      setSubmitting(true);

      const allQuestions = test.hasSections && test.sections
        ? test.sections.flatMap((s: Section) => s.questions)
        : test.questions;

      const submissionAnswers = allQuestions.map((question: Question) => ({
  ...
  }, [submitting, test, answers, startTime, onSubmit, tabSwitches, onExit]);
  ...
  // For coding-only tests, skip MCQ phase and go directly to coding
  const shouldSkipMCQPhase = !hasMCQ && hasCoding && !mcqCompleted;

  useEffect(() => {
    if (shouldSkipMCQPhase) {
      setMcqCompleted(true);
      if (test.codingQuestions && test.codingQuestions.length > 0) {
        const firstCodingId = test.codingQuestions[0]._id || test.codingQuestions[0].questionId || test.codingQuestions[0].id;
        if (firstCodingId) {
          setSelectedCodingQuestionId(firstCodingId);
        }
      }
    }
  }, [shouldSkipMCQPhase, test.codingQuestions]);

  // DEBUG: log key flags to help diagnose why coding UI may not show
  useEffect(() => {
    try {
      console.log('CleanTestInterface debug:', {
        hasMCQ,
        hasCoding,
        mcqCompleted,
        selectedCodingQuestionId,
        currentQuestionsLength: currentQuestions ? currentQuestions.length : 0
      });
    } catch (e) {
      // ignore
    }
  }, [hasMCQ, hasCoding, mcqCompleted, selectedCodingQuestionId, currentQuestions.length]);
          ...
          <div className="p-8">
            ...
            <div className="flex justify-center gap-4">
              ...
              <button
                ...
                title="Start the test in fullscreen mode"
              >
                <Monitor />
  ...
  // fullscreen violation UI was inlined elsewhere; helper removed to avoid unused symbol
        ...
        <div className="px-6 py-3">
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center gap-6">
              ...
              <button
                onClick={() => {
                  console.debug('header Submit clicked');
          ...
          <div className={`${mcqCompleted || !hasMCQ ? 'flex-1' : 'w-[75%]'} bg-white ${mcqCompleted || !hasMCQ ? '' : 'border-l'} flex flex-col h-full`}>
            ...
            <div className="flex flex-col flex-1 overflow-hidden">
              <div className="p-2 space-y-1 border-b">
                {test.codingQuestions.map((cq: CodingQuestion, idx: number) => (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-8 max-w-md w-full mx-4 shadow-2xl">
            ...
            <div className="flex justify-end gap-3">
                  <button
                    onClick={() => { console.debug('confirm modal: Cancel clicked'); setShowConfirmSubmit(false); if (startCodingFlow) setStartCodingFlow(false); }}
                    disabled={submitting}
              ...
              <button
                onClick={() => { console.debug('confirm modal: Confirm clicked'); handleSubmit(); }}
                disabled={submitting}
