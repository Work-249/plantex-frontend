...
const CleanTestInterface: React.FC<CleanTestInterfaceProps> = ({
  ...
  }, [showInstructions, currentSectionIndex, test.hasSections, test.sections, handleAutoSubmit]);
    ...
    if (mcqCompleted) {
      ...
      if (!document.fullscreenElement) {
        enterFullscreen().catch((error) => {
          console.error('Error focusing coding interface:', error);
        });
      }
      // focus coding area if available
      if (codingInterfaceRef.current) {
        (codingInterfaceRef.current as HTMLElement).focus();
      }
    }
  }, [mcqCompleted]);
  ...
  const renderFullscreenViolations = () => (
    <div className={styles.fullscreenViolations}>
      <Eye className={styles.fullscreenViolationsIcon} />
      <span className={styles.fullscreenViolationsText}>
