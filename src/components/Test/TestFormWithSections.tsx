...
interface TestFormData {
  ...
  testType: 'Assessment' | 'Practice' | 'Mock Test' | 'Specific Company Test';
  companyName?: string;
  topics: string[];
...
const TestFormWithSections: React.FC<TestFormWithSectionsProps> = ({ onSubmit, loading, initialData }) => {
  ...
  const testTypes = ['Assessment', 'Practice', 'Mock Test', 'Specific Company Test'];
  ...
  const validateForm = (): { isValid: boolean; errors: any } => {
    ...
    if (formData.hasSections) {
      ...
      // For coding-only tests, MCQ validation is not required
      if (!formData.hasCodingSection) {
    ...
    try {
      const payload = {
        ...
        codingQuestions: formData.hasCodingSection ? (formData.codingQuestions || []).map(q => ({
          ...
          points: q.points || 100,
          timeLimit: q.timeLimit || q.time_limit || 3600
        })) : [],
        sourceType: 'manual'
      };
