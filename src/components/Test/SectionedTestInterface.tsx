...
const StudentTestInterface: React.FC<StudentTestInterfaceProps> = ({
  ...
  const handleSubmit = async (isAutoSubmit = false) => {
    console.debug('SectionedTestInterface.handleSubmit called', { isAutoSubmit, submitting });
    if (submitting) return;
    ...
    if (!isAutoSubmit && unansweredCount > 0) {
      ...
      if (suppressAlerts) {
        console.debug('suppressed confirm:', confirmMessage);
      } else {
    ...
    try {
      ...
      if (suppressAlerts) {
        console.debug('suppressed alert: Failed to submit test. Please try again.');
      } else {
        ...
        <div className="max-w-6xl mx-auto px-4 py-4">
          <div className="flex justify-between items-center">
            <div>
              ...
              <button
                onClick={() => {
                  console.debug('SectionedTestInterface.header Submit clicked');
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (isPracticeMode && currentQuestion !== index && !isAnswerRevealed && answers[currentQ._id]) {
                        if (suppressAlerts) {
                          console.debug('suppressed alert: review before navigating to another question.');
                        } else {
