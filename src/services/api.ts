...
class ApiService {
  ...
  async getAssignedStudents(assignmentId: string) {
    return this.request(`/tests/assignment/${assignmentId}/assigned-students`);
  }
  ...
  async saveTestProgress(testId: string, answers: any[], startTime: Date, currentProgress: number, violations?: number) {
    return this.request(`/tests/${testId}/save-progress`, {
      method: 'POST',
      body: JSON.stringify({
        answers,
  ...
  async resumeTest(testId: string) {
    return this.request(`/tests/${testId}/resume`, {
      method: 'GET'
    });
  }
